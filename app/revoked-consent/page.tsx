...
export default async function RevokedConsent({ searchParams }: RevokedConsentProps) {
  ...
  // check if there is a study_id or uid in the url
  if (!study_id && !uid) {
    return <div className="w-full h-full flex items-center justify-center flex-col gap-y-4">
      <h1 className="text-2xl font-bold">No study_id or UID provided!</h1>
      <p className="text-sm text-muted-foreground">
        We could not revoke your consent because no study_id or UID was provided. Please contact the researcher for support.
      </p>
    </div>
  }
